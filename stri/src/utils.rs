...
/*
*
* function generateArray(s) {
    const arr = ["0"];
    let num = "10";
    for (let i = 1; i < s.length; i++) {
        arr.push(num.replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1_'));
        num = num + "0";
    }

    return "[" + arr.join(",") + "]";
}
"".split(",").map(e=> e.trim()).map(e => e.slice(1).replaceAll('0', '9').replace(/\B(?=(\d{3})+(?!\d))/g, "_")).join(", ")
*/

/*
*
* pub fn count_i64_digits_long_arr(n: i64) -> usize {
*
*  [-99, -9, 9, 99] (0 - 3)
*  usize::MAX - res.wrapping_sub(1)
   const POW10: [i64; 37] = [
      -1_000_000_000_000_000_000,
      -100_000_000_000_000_000,
      -10_000_000_000_000_000,
   ...
   let res = match POW10.binary_search(&n) {
      Ok(i) => i + 1, // exact power-> digits = index+1
      Err(i) => i,    // insertion point = number of powers <= n => digit count
   };

   if res >= 18 {
      res - 18
   } else {
      usize::MAX - res.wrapping_sub(19)
   }
}
*/

// #[inline]
// pub fn count_i8_digits(num: i8) -> usize {
//    const DIGITS_COUNTER: [i8; 4] = [-100, -10, 9, 99];
//    let i = match DIGITS_COUNTER.binary_search(&num) {
//       Ok(i) => i,
//       Err(i) => i,
//    };
//    if i >= DIGITS_COUNTER.len() / 2 {
//       i - (DIGITS_COUNTER.len() / 2 - 2)
//    } else {
//       {
...
//             num,
//             i,
//             i.wrapping_sub((DIGITS_COUNTER.len() / 2))
//          ));
//       };
//       usize::MAX - i.wrapping_sub((DIGITS_COUNTER.len() / 2)) + 3
//    }
// }

// fn report_count(num: i8) {
//    let count = count_i8_digits(num);
//    let real_count = num.to_string().len();
//    println!(
//       "num: {}, custom_count: {}, real_count: {}; {}",
//       num,
//       count,
//       real_count,
//       count == real_count
